...
[
  {
    ...
    "content": {
      "role": "model",
      "parts": [{ "text": "The weather in San Francisco is sunny and 72°F." }]
    },
    "actions": {},
...
- **Client → Server**: Send `LiveRequest` objects with user input
- **Server → Client**: Receive `Event` objects as the agent responds in real-time
...
// Display: "🔧 Agent is using get_weather tool..."
...
// Display: "✅ Weather data retrieved"
...
// Event 3: Agent's final response using the data
{
  ...
  "content": {
    ...
    "parts": [
      {
        "text": "The weather in San Francisco is sunny and 72°F with 65% humidity."
      }
    ]
  }
}
