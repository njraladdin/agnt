...
class BaseAgent(BaseModel):
  ...
  def _load_dotenv(self) -> None:
    ...
    This method tries to load .env in the following order:
    1. From the main script's location (sys.argv[0])
    2. From current working directory (searching upwards)
    3. From the agent instance's source file location (if available)
    """
    try:
      import os
      import sys
      ...
      logger.warning(
          'python-dotenv not installed. Install it with: pip install'
          ' python-dotenv'
      )
      return

    # Try loading from the main script's directory first
    # This handles: python agent_test/agent.py (run from anywhere)
    try:
      ...
      logger.debug('Could not load from script location: %s', e)

    # Try loading from current working directory
    # This handles: cd agent_test && python agent.py
    ...
    # If not found, try to infer agent instance's source location and search from there
    # This is a fallback for edge cases
    try:
      module = inspect.getmodule(self.__class__)
      if module and hasattr(module, '__file__') and module.__file__:
        module_dir = Path(module.__file__).parent
        # Walk up from module directory to find .env
        current = module_dir
        for _ in range(10):  # Search up to 10 levels
          env_file = current / '.env'
          if env_file.exists():
      ...
      logger.debug('Could not infer agent source location: %s', e)
  ...
  def run(
    ...
    """Runs the agent with a message (convenience method for testing).

    This is a synchronous convenience method that allows you to run an agent
    directly from code without setting up a runner manually. It creates an
    in-memory runner internally and handles session management automatically.

    IMPORTANT: This method is designed for testing, debugging, and simple
    scripts. For production applications, use the Runner class directly with
    proper session services and configuration.

    Args:
        message: The message to send to the agent. Can be a string or a
          types.Content object.
        user_id: User identifier for the session. Defaults to 'default_user'.
        session_id: Session identifier. Defaults to 'default_session'. Reuse
          the same ID to continue a conversation.
        app_name: Optional application name. If not provided, defaults to the
          agent's name.
        run_config: Optional run configuration for controlling agent behavior.
        load_dotenv: Whether to automatically load .env file. Defaults to True.
    ...
    Example:
        ...
        >>> for event in agent.run('What did I just say?',
    ...
    # Use agent name as app_name if not provided
    if app_name is None:
      app_name = self.name

    # Create an in-memory runner
    runner = InMemoryRunner(agent=self, app_name=app_name)
  ...
  async def run_async_simple(
    ...
    """Runs the agent with a message asynchronously (convenience method).

    This is an async convenience method that allows you to run an agent
    directly from code without setting up a runner manually. It creates an
    in-memory runner internally and handles session management automatically.
    ...
    Example:
        ...
        ...     'What did I just say?', session_id='my_session'):
        ...     if event.content:
        ...         print(event.content)
    """
    # Load .env file if requested
  ...
  def run_cli(
    ...
    """Runs the agent in an interactive CLI mode.

    This method starts an interactive command-line interface where you can
    chat with the agent. Type your messages and press Enter to send them.
    Type 'exit' to quit the interactive session.

    The interactive session maintains conversation context across messages,
    ...
    Example:
        ...
        >>> agent.run_cli()
        [user]: Hello!
        [my_agent]: Hi! How can I help you today?
        [user]: What is 2+2?
