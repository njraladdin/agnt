"""
Browser module for AI agent browser interactions
...
import time
import logging
from .page_parser import PageElement, PageParser
...
class Browser:
    """
    Browser class - Handles browser operations using SeleniumBase Driver
...
# In browser.py
    ...
    def click_element(self, selector: str) -> bool:
        ...
        try:
            ...
            scroll_success = self.scroll_to_element(selector)
            if not scroll_success:
                logger.warning(f"Could not scroll to element '{selector}', attempting click anyway")
            ...
            if selector.startswith('/') or selector.startswith('('):
                ...
                logger.info(f"Successfully clicked XPath element: {selector}")
                self.last_clicked_selector = selector
                return True
            else:
                # CSS selector - use SeleniumBase's click method
                self.driver.click(selector)
                logger.info(f"Successfully clicked CSS selector: {selector}")
                self.last_clicked_selector = selector
                return True
    ...
    def press_keys(self, selector: Optional[str], keys: Union[str, List[str]]) -> bool:
        """
            ...
            try:
                if selector:
                    if selector.startswith('/') or selector.startswith('('):
                        ...
                        self.driver.press_keys(selector, *seq)
                        return True
                else:
    ...
    def type_text(self, selector: str, text: str, clear_first: bool = True) -> bool:
        ...
        try:
            element = self.find_element(selector)
            if not element:
                logger.error(f"Element not found for selector '{selector}'")
                return False
    ...
    def scroll_to_element(self, selector: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error scrolling to element {selector}: {e}")
            return False
